...
const char MQTT_CLIMATE_MODE_STATE_TEMPLATE[][535] PROGMEM = {
  ...
  "{%set mode=value_json.SystemOperationMode|lower%}{%set is_working=states('sensor.ecodan_ashp_zone_2_working')|bool%}{%set is_prhbt=states('sensor.ecodan_ashp_zone_2_heating_prohibit')|bool or states('sensor.ecodan_ashp_zone_2_cooling_prohibit')|bool%}{%if mode=='off'%}{%if not is_prhbt and not is_working%}idle{%else%}off{%endif%}{%elif mode in ['heating','cooling','defrosting']%}{%if is_working%}{{mode}}{%elif is_prhbt%}idle{%else%}off{%endif%}{%else%}off{%endif%}",
  "{%set mode=value_json.SystemOperationMode|lower%}{%set zone_temp=state_attr('climate.zone2_climate','current_temperature')%}{%set h_prhbt=not(states('sensor.ecodan_ashp_zone_1_heating_prohibit')|bool)%}{%set c_prhbt=not(states('sensor.ecodan_ashp_zone_1_cooling_prohibit')|bool)%}{%set is_wrk=not(states('sensor.ecodan_ashp_zone_1_working')|bool)%}{%if mode=='off' and (h_prhbt or c_prhbt) and (zone_temp==0 or is_wrk)%}idle{%elif zone_temp>0 and is_wrk and (h_prhbt or c_prhbt)%}idle{%else%}{{mode}}{%endif%}",
  "{%set mode=value_json.SystemOperationMode|lower%}{%set is_working=states('sensor.ecodan_ashp_zone_2_working')|bool%}{%set is_prhbt=states('sensor.ecodan_ashp_zone_2_heating_prohibit')|bool or states('sensor.ecodan_ashp_zone_2_cooling_prohibit')|bool%}{%if mode=='off'%}{%if not is_prhbt and not is_working%}idle{%else%}off{%endif%}{%elif is_working%}{{mode}}{%elif not is_prhbt%}idle{%else%}off{%endif%}{%else%}off{%endif%}"
};
